...
### 8.2 뷰어/도구 확장
- [ ] JSON 트리 뷰
  - [ ] 트리 렌더 컴포넌트(접기/펼치기), 노드 선택 시 상세 표시
  - [ ] 입력 JSON과 동기화(포맷/업로드 후 반영)
- [ ] JSON 비교(디프)
  - [ ] A/B 입력 창 또는 탭 추가
  - [ ] 디프 결과 시각화(추가/삭제/변경 하이라이트)
- [ ] URL에서 JSON 불러오기
  - [ ] URL 입력 → fetch → 결과 입력 영역 반영
  - [ ] CORS/에러 처리, 로딩 상태/에러 표시
- [ ] JSON Schema 검증
  - [ ] Schema 입력/업로드 지원
  - [ ] 검증 결과(패스/에러 목록) UI 표시

### 8.3 사용성/품질 강화
- [ ] 실시간 포맷(디바운스) 옵션
  - [ ] 입력 지연 후 자동 포맷, 옵션 on/off
  - [ ] 포맷 실패 시 상태바 갱신 유지
- [ ] 에러 위치 하이라이트
  - [ ] 줄/컬럼 기반 강조(입력 영역 또는 코드 블록)
  - [ ] 스크롤 포커스/라인 넘버 매칭
- [ ] 접근성 개선
  - [ ] 포커스 이동/키보드 탐색, ARIA 라벨 점검
  - [ ] 색 대비/토글 상태 텍스트 보강

### 8.4 테스트/자동화
- [ ] 테스트 도입: Vitest + Vue Test Utils
  - [ ] 유틸: `parseJson`/`formatJson` (성공/실패/에러 위치)
  - [ ] 컴포넌트: 입력/출력/설정 흐름(포맷, 복사, 업로드, 들여쓰기/테마 변경)

...
- [ ] 퍼포먼스 최적화
  - [ ] 큰 JSON 렌더링 시 트리 뷰 lazy expand/가상 스크롤 검토
  - [ ] 포맷/검증 디바운스 옵션 세분화(입력/URL fetch/파일 업로드)
- [ ] 컴포넌트 모듈화
  - [ ] 버튼/상태 칩/토스트 등 UI 요소 공통 컴포넌트화
  - [ ] 설정/입력/출력 섹션 레이아웃 재사용
- [ ] 에러/로깅 일원화
  - [ ] 파싱/검증/페치 에러 메시지 포맷 통일, 사용자 노출/콘솔 로깅 기준 정리
  - [ ] Schema/Diff/Fetch 등 도구 에러를 상태바/토스트와 연계
